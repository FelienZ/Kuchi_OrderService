...
func TestUpdateProductFail(t *testing.T) {
	...
	var bodyString map[string]string
	json.Unmarshal(body, &bodyString)
	if bodyString["message"] != "failed to update product data" {
		t.Fatalf("Expected response message to equal %s, but got %s", "failed to update product data", bodyString["message"])
	}
}
